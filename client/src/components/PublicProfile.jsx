        ...
        <div className="space-y-10 w-full">
                    ...
                    <div className="space-y-2">
                        ...
                        <div>
                            ...
                            {user.user_interest.map((interest, i) => (
                    ...
                    {/* user posts */}
                     <div className=" space-y-2">
                    {userItems.some(el => !el.is_verified) && <h2 className="p-1 ring-1 px-3 py-2 w-fit">Not Verified Items</h2>}
                    {userItems
                        .filter(el => !el.is_verified)
                        .map((el, idx) => <PublicItems el={el} key={idx} />)}
                    </div>

                    <div className=" space-y-2 bg-white text-black p-2">
                    {userItems.some(el => el.is_verified) && <h2 className="p-1 ring-1 px-3 py-2 w-fit">Verified Items</h2>}
                    {userItems
                        .filter(el => el.is_verified)
                        .map((el, idx) => <PublicItems el={el} key={idx} />)}
                    </div>
