    ...
    <div className="min-w-2/5 flex flex-col justify-between ring-1 gap-2 transition-all *:transition-all text-slate-50/70 **:transition-all p-2 group shadow-md">
      <div>
        <div className="w-full max-h-[200px] overflow-hidden">
          <img
            className=" object-cover w-full group-hover:scale-105 duration-1000"
            src={data.image_url}
            alt={data.title}
          />
        </div>
        <div className="*:font-semibold text-sm">
          <Link
            to={`/item/${data.title + "=it_id" + data.id}`}
            className="text-xl text-white lg:text-2xl cursor-pointer hover:underline"
          >
            {data.title}
          </Link>
      ...
      <div className="w-full justify-between  flex gap-2 items-center">
        <p>
          Posted by:{" "}
          <Link to={`/user/${data.seller_name}`}> {data.seller_name} </Link>{" "}
        </p>
