...
const UserItems = ({ el }) => {
  const handleDelete = async (itemId) => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/user/item_delete/${itemId}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="ring-1 flex p-1 gap-4 group shadow-md hover:shadow-xl relative">
      {isEditing && <UserItemEdit el={el} handleEdit={handleEdit} />}
          ...
          <div>
            <div className="flex items-center w-full justify-between">
              <div className="flex gap-2 items-baseline">
                ...
                >
                  {el.is_verified ? "Verified" : " {waiting for verification}"}
                </p>
              </div>
            ...
            <p>
              ...
              {el.owner_id +
                el.title.slice(0, 3) +
                el.id +
                el.recent_bidder.slice(-3)}
            </p>
            <p>{el.description}</p>
            <div>
              <p>
                Current Bidder:{" "}
                {el.recent_bidder ? el.recent_bidder : "No Recent Bidders"}
              </p>
