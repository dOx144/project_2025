    ...
    return ( 
         <div className="dark:bg-[#1E1E1E] dark:text-white bg-[#e6dbdb8d] min-h-screen flex flex-col">
            ...
            <div className="ring-1 w-full p-4 flex items-center justify-between">
                ...
                <div className="space-x-5 flex">
                    {admin_dashboard.map((el, ind) => (
                        <button key={el + ind} onClick={handleClick} className=" capitalize font-semibold cursor-pointer ring-1 py-2 px-5 rounded-xl hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black ">{el}</button>
                    ))}
                </div>
            </div>
            ...
            <main className="w-full h-full ring-1 flex-grow">
                    ...
                    {/* items panel */}
                    {!isViewingUsers &&
                    <AdminItemContainer onRefresh = {fetchAdminData} data = {itemLists.result} />
                    }
