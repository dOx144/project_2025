      ...
      <div className="flex-grow flex flex-col justify-between items-center space-y-4 w-full max-w-4xl mx-auto px-4 lg:p-0">
        ...
        <main className="flex-grow flex flex-col w-full space-y-4">
          ...
          <div className="space-y-4">
            ...
            {items.length > 0 ? (
              items.map((el) => (
                <div className="flex text-slate-50/85 gap-2 ring-1 p-2 w-full" key={el.id}>
                  ...
                  <div className="flex flex-col justify-between p-1">
                    <div>
                      ...
                      <div className=" self-end-safe p-2 ring-1">
                        <Link to={`/user/${el.seller_name}`}>Posted by: {el.seller_name}</Link>
                        <p>Status: {el.is_verified ? "Verified 🟢" : "Not Verified 🔴"}</p>
                        <p>Your Bid: Rs. {el.current_price}</p>
                        <p>Ends at: {new Date(el.ends_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="text-sm   ">
                      {new Date(el.ends_at) > new Date() ? (
                        <p className="text-green-50/70 ">Your Item ID will be generated for the trade if you are the last Bidding person.</p>
                      ) : (
                        <p>
                          Your Item Id is {el.owner_id + el.title.slice(0, 3) + el.id + el.current_bidder.slice(-3)}
                        </p>
                      )}
              ...
              <div className="text-gray-500 text-center mt-4">No current bids 😕</div>
            )}
          </div>
        </main>
      </div>
