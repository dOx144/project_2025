...
app.post("/api/users/updateInterest", async (req, res) => {
  ...
  if (!userId || !Array.isArray(interests) || interests.length === 0) {
    return res.status(400).json({
      message:
        "Invalid request: userId and non-empty interests array are required",
    });
  }
...
app.post("/api/upload_profile", upload.single("file"), async (req, res) => {
  try {
    ...
    const myQuery = await pool.query(
      "INSERT INTO items(name, description, title) VALUES ($1, $2, $3) RETURNING *",
...
app.delete("/api/user/item_delete/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .send({ error: "Item not found or already deleted." });
    }
